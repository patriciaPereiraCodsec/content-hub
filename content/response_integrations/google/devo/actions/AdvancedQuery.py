...
from ..core.utils import get_start_end_timestamp
...
def main():
    ...
    try:
        ...
        siemplify.LOGGER.info(
            "Executing an advanced query based on the provided parameters"
        )
        query_result = manager.run_advanced_query(params=params)
        siemplify.LOGGER.info(
            "Successfully Executed an advanced query based on the provided parameters"
        )
        ...
        if query_result.objects:
            ...
            output_message = (
                f"No results found for the provided query in {INTEGRATION_DISPLAY_NAME}"
            )
