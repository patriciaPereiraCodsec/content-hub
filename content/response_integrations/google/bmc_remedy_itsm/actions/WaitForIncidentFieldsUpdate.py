...
from ..core.UtilsManager import is_async_action_timeout
...
def check_incident_fields(incident, fields_to_check_dict):
    ...
    if not_found_fields:
        raise Exception(
            f"the following fields were not found in the structure of the incident: "
...
def main():
    ...
    fields_to_check = extract_action_param(
        siemplify, param_name="Fields To Check", print_value=True
    )
    ...
    try:
        ...
        if is_async_action_timeout(siemplify):
            ...
            if not fail_if_timeout:
                raise BMCRemedyITSMTimeoutException(
                    f"The following fields were not updated for incident with ID "
                    f"{incident_id} in {INTEGRATION_DISPLAY_NAME}: "
                    f"{','.join(additional_data)}"
                )

            raise Exception(
                f"The following fields were not updated for incident with ID {incident_id} in "
                f"{INTEGRATION_DISPLAY_NAME}: {','.join(additional_data)}"
            )

        try:
            fields_to_check_dict = (
                json.loads(fields_to_check) if fields_to_check else {}
            )
        except Exception:
            raise Exception(
                'Invalid JSON payload provided in the parameter "Fields To Check". Please check the '
                "structure."
            )

        if not STATUS_MAPPING.get(incident_status) and not fields_to_check_dict:
            raise Exception(
                '"Status" or "Fields To Check" parameter should have a value.'
            )
        ...
        updated_fields, not_updated_fields = check_incident_fields(
            incident, fields_to_check_dict
        )
        ...
        if len(updated_fields) == len(fields_to_check_dict.keys()):
            ...
            output_message = (
                f"Waiting for the following fields to be updated for incident with ID {incident_id} in "
        ...
        siemplify.LOGGER.error(
            f"General error performing action {WAIT_FOR_INCIDENT_FIELDS_UPDATE_SCRIPT_NAME}"
        )
