...
SCRIPT_NAME = "Panorama - BlockURLs"
...
def main():
    ...
    for entity in siemplify.target_entities:
        if entity.entity_type == EntityTypes.URL:
            urlToBlock.append(
                entity.identifier.replace("&amp;", "&").replace("&", "&amp;")
            )
    ...
    if urlToBlock:
        ...
        for url in urlToBlock:
            siemplify.LOGGER.info(f"Process entity: {url} Started")
            try:
                siemplify.LOGGER.info(f"Adding url {url} to blacklist")
                api.AddBlockedUrls(
                ...
                siemplify.LOGGER.info(f"Successfully blacklisted url {url}")
                successful_entities.append(url)
            except Exception as error:
                siemplify.LOGGER.error(f"Failed to block url {url}")
        ...
        if successful_entities:
            output_message += (
                'Successfully added the following URLs to the Palo Alto Panorama URL Category "{}": '
                "{}".format(
        ...
        if not successful_entities:
            output_message = f'No URLs were added to the Palo Alto Panorama URL Category "{category}"'
            result_value = "false"
