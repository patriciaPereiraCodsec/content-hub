...
SCRIPT_NAME = "Panorama - AddIpsToGroup"
...
def main():
    ...
    if ipsToAdd:
        ...
        for ip in ipsToAdd:
            ...
            try:
                siemplify.LOGGER.info(f"Adding entity: {ip} to group: {group}")
                ...
                siemplify.LOGGER.info(
                    f"Successfully added entity: {ip} to group: {group}"
                )
            except Exception as error:
                siemplify.LOGGER.error(
                    f"Unable to add entity: {ip}. Reason is: {error}"
                )
        ...
        if successful_entities:
            output_message += (
                'Successfully added the following IPs to the Palo Alto Panorama address group "{}": '
                "{}".format(
        ...
        if not successful_entities:
            output_message = (
                f'No IPs were added to the Palo Alto Panorama address group "{group}"'
            )
            result_value = "false"
