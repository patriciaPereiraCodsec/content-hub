...
def main():
    ...
    try:
        ...
        manager.release_quarantined_email(sensor_name=sensor_name, queue_id=queue_id)
        ...
        output_message = (
            f'Error executing action "Release Quarantined Email". Reason: {error}'
        )
        ...
        output_message = (
            f'Email with queue id "{queue_id}" was not released. Reason: {error}'
        )
        siemplify.LOGGER.error(error)
        siemplify.LOGGER.exception(error)
