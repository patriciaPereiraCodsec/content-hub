...
def main():
    ...
    try:
        ...
        siemplify.LOGGER.info(
            f"Successfully retrieved information about the policy of the {bucket_name} "
            "bucket in AWS S3"
        )
        output_message += (
            f"Successfully retrieved information about the policy of the {bucket_name} "
        ...
        json_results.append({"Policy": bucket_policy.to_dict()})

    except AWSS3StatusCodeException as error:
        result_value = "false"
        siemplify.LOGGER.error(
            "Action wasn’t able to return information about the policy of the "
            f"{bucket_name} bucket in AWS S3"
        )
        ...
        siemplify.LOGGER.error(
            f"Error executing action 'Get Bucket Policy'. Reason: {error}"
        )
        siemplify.LOGGER.exception(error)
