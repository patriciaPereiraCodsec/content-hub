...
def main():
    ...
    bucket_policy = extract_action_param(
        siemplify, param_name="Policy JSON Object", is_mandatory=True, print_value=True
    )
    ...
    try:
        ...
        s3_client.set_bucket_policy(
            bucket_name=bucket_name, bucket_policy=bucket_policy
        )
        siemplify.LOGGER.info(
            f"Successfully updated policy of the {bucket_name} bucket in AWS S3"
        )
        output_message += (
            f"Successfully updated policy of the {bucket_name} bucket in AWS S3"
        )
        ...
        siemplify.LOGGER.error(
            f"Action wasn’t able to update the policy of the {bucket_name} bucket in "
            f"AWS S3"
        )
        ...
        siemplify.LOGGER.error(
            f"Error executing action 'Set Bucket Policy'. Reason: {error}"
        )
        siemplify.LOGGER.exception(error)
