...
HASH_TYPE_KEYS = {"md5": HashType.MD5, "sha1": HashType.SHA1, "sha256": HashType.SHA256}
...
class McAfeeTIEDXLManager:
    def __init__(
        self, server_address, broker_ca_bundle_path, cert_file_path, private_key_path
    ):
        # Create config
    ...
    def get_file_reputation(self, filehash):
        ...
        if not hash_type_key:
            raise McAfeeTIEDXLManagerError(f"Unsupported hash type - {hash_type}")
        ...
        reputations = [
            self.parse_reputation(reputation) for reputation in list(res.values())
        ]
        return reputations
    ...
    def set_file_reputation(self, filehash, trust_level, filename=None, comment=None):
        ...
        if not trust_level_key:
            raise McAfeeTIEDXLManagerError(f"Unsupported trust level - {trust_level}")
        ...
        try:
            # Set the reputation
            self.tie_client.set_file_reputation(
                trust_level_key, hash_param, filename, comment
            )
            return True

        except Exception as e:
            raise McAfeeTIEDXLManagerError(
                f"Unable to set reputation for {filehash}: {str(e)}"
            )
    ...
    def prepare_enrichment_repustations(self, reputations):
        ...
        for rep in reputations:
            ...
            rep_dict = {
                f"{provider}_trust_level": int(rep.get("Trust Level")),
                f"{provider}_vendor": rep.get("Verbose Provider"),
                f"{provider}_first_contact": rep.get("First Contact"),
                f"{provider}_verbose_trust_level": rep.get("Verbose Trust level"),
                f"{provider}_provider_id": rep.get("Provider ID"),
            }
    ...
    def parse_reputation(rep):
        ...
        for trust_level_name, trust_level_value in TRUST_LEVELS.items():
        ...
        provider = PROVIDERS.get(
            str(provider_id).encode("utf-8"), str(provider_id).encode("utf-8")
        )
