...
from ..core.constants import INTEGRATION_NAME, ADD_COMMENT_TO_INSIGHT_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        json_response = manager.add_comment_to_insight(
            insight_id=insight_id, comment=comment
        )
        ...
        siemplify.LOGGER.error(
            f"General error performing action {ADD_COMMENT_TO_INSIGHT_SCRIPT_NAME}"
        )
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = (
            f"Error executing action {ADD_COMMENT_TO_INSIGHT_SCRIPT_NAME}. Reason: {e}"
        )
