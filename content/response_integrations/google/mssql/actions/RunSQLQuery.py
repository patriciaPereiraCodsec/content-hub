...
RESULT_TABLE_HEADER = "MSSQL Query Results"
...
def main():
    ...
    try:
        ...
        # Run search query
        results = sql_manager.execute_query(query, commit) or {}
        if commit:
            siemplify.LOGGER.info(f"Query {query} have been committed")

        siemplify.LOGGER.info(f"Found {len(results)} results.")

        if results:
            # Add results as CSV
            siemplify.LOGGER.info("Attaching results as CSV table.")
            siemplify.result.add_data_table(RESULT_TABLE_HEADER, construct_csv(results))
            output_message = "Successfully finished search."
