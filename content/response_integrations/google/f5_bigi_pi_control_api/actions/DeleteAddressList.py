...
def main():
    ...
    try:
        ...
        output_message = (
            f"Address list {list_name} doesn't exist in {INTEGRATION_DISPLAY_NAME}."
        )

    except Exception as e:
        siemplify.LOGGER.error(
            f"General error performing action {DELETE_ADDRESS_LIST_NAME}"
        )
        siemplify.LOGGER.exception(e)
        status = EXECUTION_STATE_FAILED
        result = False
        output_message = (
            f'Error executing action "{DELETE_ADDRESS_LIST_NAME}". Reason: {e}'
        )
