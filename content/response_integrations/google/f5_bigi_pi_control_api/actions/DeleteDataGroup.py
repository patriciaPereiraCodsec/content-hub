...
from ..core.constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, DELETE_DATA_GROUP_NAME
...
def main():
    ...
    try:
        ...
        output_message = (
            f"Data group {group_name} doesn't exist in {INTEGRATION_DISPLAY_NAME}."
        )

    except Exception as e:
        siemplify.LOGGER.error(
            f"General error performing action {DELETE_DATA_GROUP_NAME}"
        )
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = (
            f'Error executing action "{DELETE_DATA_GROUP_NAME}". Reason: {e}'
        )
