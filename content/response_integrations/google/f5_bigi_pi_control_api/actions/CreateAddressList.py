...
def main():
    ...
    try:
        ...
        address_list = manager.create_address_list(
            list_name=list_name, addresses=addresses
        )
        ...
        siemplify.LOGGER.error(
            f"General error performing action {CREATE_ADDRESS_LIST_NAME}"
        )
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = (
            f'Error executing action "{CREATE_ADDRESS_LIST_NAME}". Reason: {e}'
        )
