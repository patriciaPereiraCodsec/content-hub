...
def main():
    ...
    limit = extract_action_param(
        siemplify,
        param_name="Max Port Lists To Return",
        input_type=int,
        print_value=True,
    ...
    try:
        ...
        if filter_value:
            if filter_logic == EQUAL_FILTER:
                port_lists = [
                    ...
                    if port_list.name == filter_value
                ]
            elif filter_logic == CONTAINS_FILTER:
                port_lists = [
                    port_list
                    for port_list in port_lists
                    if filter_value in port_list.name
                ]
        ...
        if port_lists:
            siemplify.result.add_data_table(
                TABLE_NAME,
                construct_csv([port_list.to_csv() for port_list in port_lists]),
            )
            siemplify.result.add_result_json(
                [port_list.to_json() for port_list in port_lists]
            )
            output_message = (
                f"Successfully found port lists for the provided criteria in"
                f" {INTEGRATION_NAME}."
            )
        else:
            result_value = False
        ...
        output_message = (
            f"Error executing action {LIST_PORT_LISTS_SCRIPT_NAME}. Reason: {e}"
        )
