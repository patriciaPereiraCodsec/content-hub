...
def main():
    ...
    try:
        ...
        output_message = (
            f"Successfully updated iRule {name} in {INTEGRATION_DISPLAY_NAME}"
        )

    except Exception as e:
        siemplify.LOGGER.error(
            f"General error performing action {UPDATE_IRULE_SCRIPT_NAME}"
        )
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = (
            f'Error executing action "{UPDATE_IRULE_SCRIPT_NAME}". Reason: {e}'
        )
