...
from ..core.constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, CREATE_DATA_GROUP_NAME
...
def main():
    ...
    try:
        ...
        data_group = manager.create_data_group(
        ...
        siemplify.LOGGER.error(
            f"General error performing action {CREATE_DATA_GROUP_NAME}"
        )
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = (
            f'Error executing action "{CREATE_DATA_GROUP_NAME}". Reason: {e}'
        )
