...
def get_rules(manager, rule_name):
    ...
    if existing_rule:
        ...
        output_message = (
            f'"{rule_name}" rule was not found, new rule will be created. Continuing...'
        )
    ...
    siemplify.result.add_result_json(
        {
            ...
            "already_available": list(
                set(existing_domains).intersection(valid_domains)
            ),
...
def main():
    ...
    rule_name = extract_action_param(
        siemplify,
        param_name="Rule to add Domains to",
        is_mandatory=True,
        print_value=True,
    )
    ...
    try:
        ...
        output_message = (
            f'Error performing "{ADD_DOMAINS_TO_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME}" '
            f"action: {e}"
        )
