...
from soar_sdk.SiemplifyUtils import output_handler, extract_domain
...
from ..core.UtilsManager import validate_email_address
...
def get_rules(manager, rules):
    ...
    result_value = {"get_rules": COMPLETED_STATUS}
    ...
    if existing_rules:
        output_message = "Found {} rules. Continuing...".format(
            PARAMETERS_DEFAULT_DELIMITER.join(
                [f'"{existing_rule.name}"' for existing_rule in existing_rules]
            )
        )
    else:
...
def add_items_to_rule(
    siemplify, manager, rules, valid_items, invalid_items, add_domains, additional_data
):
    """
    Add items to rule
    ...
    if len(rules) == len(processed_rules):
        siemplify.result.add_result_json(
            {
                ...
                "already_available": list(
                    set(existing_items).intersection(valid_items)
                ),
        ...
        output_message = prepare_output_messages(
            rules, valid_items, invalid_items, add_domains
        )
    else:
        additional_data.update({"processed_rules": processed_rules})
...
def main():
    ...
    # Action parameters
    email_addresses_string = extract_action_param(
        siemplify, param_name="Email Addresses", print_value=True
    )
    rule_name = extract_action_param(
        siemplify,
        param_name="Rule to add senders to",
        is_mandatory=True,
        print_value=True,
    )
    add_domains = extract_action_param(
        siemplify,
        param_name="Should add senders' domain to the corresponding Domains "
        "List rule as well?",
        input_type=bool,
        print_value=True,
    )
    ...
    email_addresses = (
        [
            email_address.strip()
            for email_address in email_addresses_string.split(
                PARAMETERS_DEFAULT_DELIMITER
            )
    ...
    try:
        ...
        if not email_addresses:
            siemplify.LOGGER.info(
                "No email addresses provided in action parameters. Action will work with User "
                "entities"
            )
            email_addresses = [
                entity.identifier
                ...
                and validate_email_address(entity.identifier)
            ]
        ...
        invalid_email_addresses = list(
            set(email_addresses) - set(valid_email_addresses)
        )
        ...
        if additional_data.get("get_rules") != COMPLETED_STATUS:
            ...
            output_message, result_value, status = add_items_to_rule(
                siemplify,
                manager,
                rules,
                valid_email_addresses,
        ...
        output_message = (
            'No valid email addresses provided in "Email Addresses" parameter, or there are no valid '
        ...
        output_message = (
            f'Error performing "{ADD_SENDERS_TO_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULE_SCRIPT_NAME}" '
            f"action : {e}"
        )
...
def prepare_output_messages(
    rules, valid_email_addresses, invalid_email_addresses, add_domains=None
):
    """
    ...
    """
    output_message = (
        f"Added the following inputs to the corresponding rules: "
