...
SCRIPT_NAME = "CBResponse - Download Binary"
        ...
        if target_entities:
            for entity in target_entities:
                ...
                try:
                    ...
                    base64_content = base64.b64encode(binary_content).encode()

                    siemplify.result.add_entity_attachment(
                        entity.identifier, f"{entity.identifier}.zip", base64_content
                    )
                    json_results[entity.identifier] = base64_content

                    successful_entities.append(entity)
                    output_message += f"Downloaded binary for the following hash:{entity.identifier} \n"
                    siemplify.LOGGER.info(
                        f"Finished processing entity {entity.identifier}"
                    )
                except Exception as e:
                    output_message += f"Unable to download binary for the following entity {entity.identifier}: \n"
