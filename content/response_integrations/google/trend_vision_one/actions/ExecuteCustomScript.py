...
from ..core.datamodels import AgentResult
from ..core.TrendVisionOneExceptions import (
...
def run_script_on_agents(
    ...
    """Run a custom script on a list of agents.

    Args:
        manager (TrendVisionOneManager): An instance of the TrendVisionOneManager for
        interacting with the API.
        script_name (str): The name of the script to run.
        script_parameters (str): Parameters to pass to the script.
        agent_uuids (list[str]): A list of agent UUIDs to run the script on.
        result_data (dict[str, Any]): A dictionary to store the results of the
        operation.
    """
    ...
    result_data["failed"].extend(agents.failed_agents)
    for endpoint in agents.successful_agents:
        if endpoint.guid in result_data["result_endpoints"]:
            result_data["result_urls"][endpoint.guid] = result_data[
                "result_endpoints"
            ][endpoint.guid]
            ...
            try:
                ...
                if endpoint.guid not in result_data["failed"]:
                    result_data["failed"].append(endpoint.guid)
                manager.siemplify.LOGGER.error(
                    f"Run Script failed on Agent {endpoint.guid}"
                )
                manager.siemplify.LOGGER.exception(e)
                continue
            result_data["result_urls"][endpoint.guid] = result_url
...
def start_operation(
    ...
    """ Start the operation of running a custom script on endpoints and agents.

    Args:
        siemplify (SiemplifyAction): The Siemplify action instance.
        manager (TrendVisionOneManager): An instance of the TrendVisionOneManager for
        interacting with the API.
        action_start_time (int): The start time of the action in Unix time.
        script_name (str): The name of the script to run.
        script_parameters (str): Parameters to pass to the script.
        suitable_entities (List): A list of suitable entities to run the script on.
        agent_uuids (list[str]): A list of agent UUIDs to run the script
        on.
    Returns:
        Tuple[str, bool, int]: A tuple containing the output message, result value,
    ...
    for entity in suitable_entities:
        ...
        try:
            ...
            if endpoint:
                if endpoint.guid in result_data["result_endpoints"]:
                    result_data["result_urls"][entity.identifier] = result_data[
                        "result_endpoints"
                    ][endpoint.guid]
                else:
                    result_url = manager.run_script(
                ...
                siemplify.LOGGER.info(
                    f"No endpoint found for the entity: {entity.identifier}"
                )
...
def query_operation_status(
    ...
    for entity_identifier, result_url in results_urls.items():
        ...
        if (
            ...
            < GLOBAL_TIMEOUT_THRESHOLD_IN_MIN * 60_000
    ...
    if any(result_data["result_urls"].values()):
        output_message = f"Pending endpoints: {', '.join( [ key for key, value in result_data['result_urls'].items() if value ] )}"
...
def main(is_first_run):
    ...
    script_name = extract_action_param(
        siemplify, param_name="Script Name", print_value=True, is_mandatory=True
    )
    script_parameters = extract_action_param(
        siemplify, param_name="Script Parameters", print_value=True
    )
    ...
    try:
        if not suitable_entities and not agent_uuids:
            ...
            script = manager.get_script_by_name(script_name=script_name)
        ...
        output_message = (
            f"Error executing action {EXECUTE_CUSTOM_SCRIPT_SCRIPT_NAME}. Reason: {e}\n"
        )
