...
from ..core.constants import PROVIDER_NAME, RUN_SAVED_QUERY_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        if results:
            ...
            output_message = (
                f"{PROVIDER_NAME} saved query {saved_query_id} executed successfully"
            )
        else:
            output_message = f"{PROVIDER_NAME} saved query {saved_query_id} executed successfully, but didn't return any results"
    except NotFoundException as e:
        result_value = False
        output_message = f"Failed to run {PROVIDER_NAME} saved query. Error is: {e}"
    except Exception as e:
        siemplify.LOGGER.error(
            f"General error performing action {RUN_SAVED_QUERY_SCRIPT_NAME}"
        )
