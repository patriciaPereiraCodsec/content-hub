...
from ..core.constants import (
    INTEGRATION_NAME,
    REMOVE_URL_FROM_GROUP_SCRIPT_NAME,
...
def main():
    ...
    try:
        ...
        if successful_entities:
            output_message += f"Successfully removed the following URLs from the {group_name} Checkpoint FireWall Group: {PARAMETERS_NEW_LINE_DELIMITER.join(successful_entities)}\n"

        if failed_entities:
            output_message += (
                "Action wasn’t able to remove the following URLs from the {} Checkpoint FireWall Group: "
                "{}\n".format(
        ...
        if not successful_entities:
            output_message = (
                f"No URLs were removed from the {group_name} Checkpoint FireWall Group."
            )
            result_value = False
        ...
        output_message = f"No URLs were removed from the {group_name} Checkpoint FireWall Group. Reason: {err}"
        result_value = False
