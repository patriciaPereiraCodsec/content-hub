...
def filter_items(
    items, filter_key=None, filter_logic=None, filter_value=None, limit=None
):
    """
    ...
    if (
        ...
        items = [
            item
            for item in items
            if FILTER_STRATEGY_MAPPING[filter_logic](
                getattr(item, FILTER_KEY_MAPPING.get(filter_key)), filter_value
            )
        ]
