...
def main():
    ...
    try:
        ...
        if success_entities:
            ...
            output_message = (
                "Successfully retrieved agent version information from the following endpoints in "
                f'{PRODUCT_NAME}: {", ".join(success_entities)}\n'
            )
            if failed_entities:
                output_message += (
                    "Action wasn't able to retrieve agent version information from the following "
