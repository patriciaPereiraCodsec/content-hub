...
from ..core.constants import (
    INTEGRATION_NAME,
    GET_LAST_COMMUNICATION_TIME_SCRIPT_NAME,
    GET_LAST_COMMUNICATION_TIME_TABLE_NAME,
    PRODUCT_NAME,
)
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                last_communication_time = manager.get_last_comm_time(
                    entity_original_identifier
                )
                json_results[entity.identifier] = last_communication_time.to_json()
                csv_output.append(
                    last_communication_time.to_csv(entity_original_identifier)
                )
        ...
        if successful_entities:
            ...
            siemplify.result.add_data_table(
                GET_LAST_COMMUNICATION_TIME_TABLE_NAME, construct_csv(csv_output)
            )
            output_message = (
                "Successfully retrieved last communication time information from the "
                f'following endpoints in {PRODUCT_NAME}: {", ".join(successful_entities)}\n'
            )
            if failed_entities:
                output_message += (
                    "Action wasn't able to retrieve last communication time information from the "
