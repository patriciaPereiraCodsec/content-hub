...
from ..core.constants import GET_DAT_VERSION_SCRIPT_NAME, INTEGRATION_NAME, PRODUCT_NAME
...
def main():
    ...
    try:
        ...
        if success_entities:
            ...
            output_message = (
                "Successfully retrieved DAT information from the following endpoints in "
                f'{PRODUCT_NAME}: {", ".join(success_entities)}\n'
            )
            if failed_entities:
                output_message += (
                    "Action wasn't able to retrieve DAT information from the following endpoints "
        ...
        output_message = (
            f"Error executing action '{GET_DAT_VERSION_SCRIPT_NAME}'. Reason: {e}"
        )
