...
from ..core.consts import INTEGRATION_NAME, HUNT_URL_PART, GET_HUNT_DETAILS
from ..core.exceptions import GoogleGRRNotFoundException


@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = f"{INTEGRATION_NAME} - {GET_HUNT_DETAILS}"
    ...
    try:
        ...
        for hunt_id in hunt_ids:
            try:
                ...
                siemplify.result.add_link(
                    f"Hunt {hunt_id} Link: ", f"{api_root}{HUNT_URL_PART}{hunt_id}"
                )

            except GoogleGRRNotFoundException as e:
                invalid_hunts.append(hunt_id)
                siemplify.LOGGER.info(
                    f"Failed to fetch hunt with id: {hunt_id}. Error: {e}"
                )
        ...
        if invalid_hunts:
            output_message += (
                f"Could not fetch details for the specified hunts: {', '.join(invalid_hunts)} "
