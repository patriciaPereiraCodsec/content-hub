...
def start_operation(siemplify, manager, suitable_entities, comment):
        ...
        for entity in suitable_entities:
            try:
                ...
                if initial_isolation_status == ISOLATED:
                    siemplify.LOGGER.info(
                        f"Endpoint for entity {entity.identifier} is already isolated. "
                        "Skipping."
                    )
                    ...
                    manager.isolate_or_unisolate_endpoint(
                        isolate=True, endpoint_id=endpoint.scan_id, comment=comment
                    )
    ...
    if result_value["pending"]:
        output_message = (
            "Waiting for isolation to finish on the following entities: "
...
def finish_operation(siemplify, result_data, timeout_approaching):
    ...
    if already_completed:
        output_message += (
            "The following endpoints were already isolated in {}: "
            "{}\n".format(
                INTEGRATION_DISPLAY_NAME,
    ...
    if timeout_approaching and pending_entities:
        ...
        error_message = (
            f"Error executing action {ISOLATE_ENDPOINT_SCRIPT_NAME}. Reason: {err}"
        )
