...
from ..core.utils import load_json_string_to_dict
...
def main():
    ...
    siemplify.script_name = (
        f"{INTEGRATION_IDENTIFIER} - {UPDATE_TICKET_TIME_ENTRY_SCRIPT_NAME}"
    )
    ...
    note = extract_action_param(
        siemplify, param_name="Note", print_value=True, is_mandatory=False
    )
    time_spent = extract_action_param(
        siemplify, param_name="Time Spent", print_value=True, is_mandatory=False
    )
    billable = extract_action_param(
        siemplify,
        param_name="Billable",
        print_value=True,
        default_value=False,
    ...
    try:
        ...
        if agent_email:
            ...
            if agent_email not in agents:
                raise FreshworksFreshserviceMissingAgentError(
                    f"Agent with email {agent_email} was not found"
        ...
        try:
            ...
            output_message = (
                f"Time entry {time_entry_id} is updated for the ticket {ticket_id}."
            )
            result_value = True
        except FreshworksFreshserviceValidationError as error:
            if TIME_SPENT_FIELD in f"{error}":
                output_message = (
                    f"Specified time spent value {time_spent} is in incorrect format,"
                    " it should be in the 'hh:mm' format."
                )
            else:
        ...
        output_message = (
            "Failed to update ticket time entry with the provided parameters!"
            f" Error is: {error}"
        )
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)
        ...
        output_message = (
            f"Error executing action '{UPDATE_TICKET_TIME_ENTRY_SCRIPT_NAME}'."
            f"Reason: {error}"
        )
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)
