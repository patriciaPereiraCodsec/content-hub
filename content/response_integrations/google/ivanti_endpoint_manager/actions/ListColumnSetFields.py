...
def main():
    ...
    # Action parameters
    column_set = extract_action_param(
        siemplify, param_name="Column Set", is_mandatory=True, print_value=True
    )
    ...
    limit = extract_action_param(
        siemplify, param_name="Max Fields To Return", input_type=int, print_value=True
    )
    ...
    try:
        ...
        fields = manager.get_column_set_fields(
            column_set, filter_logic, filter_value, limit
        )
        ...
        if fields:
            ...
            output_message += (
                f'Successfully found columns set "{column_set}" fields for the provided criteria in'
                f" {INTEGRATION_DISPLAY_NAME}."
            )
        else:
            output_message = (
                f'No column set "{column_set}" fields were found for the provided criteria in'
                f" {INTEGRATION_DISPLAY_NAME}."
            )

    except Exception as e:
        siemplify.LOGGER.error(
            f"General error performing action {LIST_COLUMN_SET_FIELDS_SCRIPT_NAME}"
        )
