...
def main():
    ...
    insight_name = extract_action_param(
        siemplify, param_name="Insight Name", is_mandatory=True, print_value=True
    )

    group_by_attribute = extract_action_param(
        siemplify, param_name="Group By Attribute", is_mandatory=True, print_value=True
    )

    filter_json = extract_action_param(
        siemplify, param_name="Filter JSON Object", is_mandatory=True
    )
    ...
    try:
        ...
        insight_arn = hub_client.create_insight(
        ...
        siemplify.LOGGER.info(
            f"Successfully created {insight_name} in AWS Security Hub"
        )
        ...
        siemplify.LOGGER.error(
            f"Action wasn’t able to create {insight_name} insight. Reason: {error}"
        )
        ...
        siemplify.LOGGER.error(
            f"Error executing action 'Create Insight'. Reason: {error}"
        )
        siemplify.LOGGER.exception(error)
