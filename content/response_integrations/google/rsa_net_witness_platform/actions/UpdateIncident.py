...
def main():
    ...
    incident_status = extract_action_param(
        siemplify, param_name="Status", input_type=str, is_mandatory=False
    )
    assignee = extract_action_param(
        siemplify, param_name="Assignee", input_type=str, is_mandatory=False
    )
    ...
    try:
        ...
        incident = rsa_manager.update_incident(
            incident_id=incident_id, status=incident_status, assignee=assignee
        )
        output_message = (
            f"Successfully updated incident with ID {incident_id} in RSA Netwitness"
        )
        result_value = True
        siemplify.result.add_result_json(incident.to_json())
    except UpdateFailException as e:
        output_message = f"Action wasn't able to update incident with ID {incident_id} in RSA Netwitness. Reason: {e}"
