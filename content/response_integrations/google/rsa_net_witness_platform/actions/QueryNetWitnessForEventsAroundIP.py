    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                events = rsa_manager.get_events_for_ip(
                ...
                if events:
                    ...
                    siemplify.result.add_entity_attachment(
                        ...
                        rsa_manager.get_pcap_for_ip(entity.identifier, hours_backwards),
                    )
        ...
        if failed_entities:
            output_message += f'Events were not found for the following IPs: {", ".join(failed_entities)}.'

        if entities_with_result:
            output_message += f'Successfully found events in RSA NetWitness for the following IPs: {", ".join(entities_with_result)}.'
        else:
        ...
        output_message = f"Error executing action {QUERY_NET_IP_ACTION}. Reason: {e}"
