    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                events = rsa_manager.get_events_for_domain(
                ...
                if events:
                    ...
                    siemplify.result.add_entity_attachment(
                        ...
                        rsa_manager.get_pcap_for_domain(
                            entity.identifier, hours_backwards
                        ),
                    )
        ...
        if failed_entities:
            output_message += f'Events were not found for the following hostnames: {", ".join(failed_entities)}.'

        if entities_with_result:
            output_message += f'Successfully found events in RSA NetWitness for the following hostnames: {", ".join(entities_with_result)}.'
        else:
        ...
        output_message = f"Error executing action {QUERY_NET_HOST_ACTION}. Reason: {e}"
