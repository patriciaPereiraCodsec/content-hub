...
SCRIPT_NAME = "McAfeeMvisionEDR - Stop And Remove Content"
...
def start_operation(siemplify, manager, pid, full_file_path):
    ...
    output_message = f"Process with PID {pid} wasn't stopped and {full_file_path} wasn't removed from any entities."
...
def query_operation_status(
    ...
    if not current_entity:
        current_entity = process_next_entity(
            siemplify, entities_to_process, manager, pid, full_file_path
        )
    ...
    if action_status == COMPLETED_STATUS:
        ...
        output_message = f"Successfully stopped process with PID {pid} and removed {full_file_path} from the following entity: {current_entity.get('entity')}"
    elif action_status == ERROR_STATUS:
        failed_entities.append(current_entity.get("entity"))
        output_message = (
            "Action wasn't able to stop process with PID {} and remove {} from the following "
            "entity: {}".format(pid, full_file_path, current_entity.get("entity"))
        )
        ...
        output_message = f"Action wasn't able to stop the process with PID {pid} and remove {full_file_path} from {fail_entity.get('entity')}."
        if errors.descriptions:
    ...
    if processed_entities:
        result_value = "true"
        output_message += "Successfully stopped process with PID {} and removed {} from the following entities: {}".format(
            pid,
            full_file_path,
    ...
    if failed_entities:
        output_message += "\n\nAction wasn't able to stop process with PID {} and remove {} from the following entities: {}".format(
            pid,
            full_file_path,
    ...
    if failed_entities_with_reason:
        for item in failed_entities_with_reason:
            output_message += f"\n\nAction wasn't able to stop the process with PID {pid} and remove {full_file_path} from {item.get('entity')}."
            if item.get("reason"):
...
def process_next_entity(siemplify, entities_to_process, manager, pid, full_file_path):
    ...
    try:
        ...
        task_id = manager.stop_and_remove_content(
            host_id, pid, full_file_path
        ).status_id
...
def main(is_first_run):
    ...
    # Parameters
    pid = extract_action_param(
        siemplify, param_name="PID", is_mandatory=True, input_type=int
    )
    ...
    try:
        ...
        if is_first_run:
            output_message, result_value, status = start_operation(
                siemplify, mvision_edr_manager, pid, full_file_path
            )
        else:
            (
                processed_entities,
