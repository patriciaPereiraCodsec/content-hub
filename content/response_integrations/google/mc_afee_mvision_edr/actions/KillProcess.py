...
SCRIPT_NAME = "McAfeeMvisionEDR - Kill Process"
...
def start_operation(siemplify, manager, pid_type, pid_value):
    ...
    :param pid_type: {str} Process identifier type.
    :param pid_value: {str} The value for the process identifier.
    ...
    output_message = f"Action wasn't able to stop the process with {pid_type} {pid_value} on any entities."
...
def query_operation_status(
    ...
    if not current_entity:
        current_entity = process_next_entity(
            siemplify, entities_to_process, manager, pid_type, pid_value
        )
    ...
    if action_status == COMPLETED_STATUS:
        ...
        output_message += f"Successfully stopped process with {pid_type} {pid_value} from the following entity: {current_entity.get('entity')}"
    elif action_status == ERROR_STATUS:
        failed_entities.append(current_entity.get("entity"))
        output_message += f"Action wasn't able to stop the process with {pid_type} {pid_value} from: {current_entity.get('entity')}"
        ...
        output_message += f"Action wasn't able to stop the process with {pid_type} {pid_value} from {fail_entity.get('entity')}."
        if errors.descriptions:
    ...
    if processed_entities:
        result_value = "true"
        output_message += "Successfully stopped process with {} {} from the following entities: {}".format(
            pid_type,
            pid_value,
    ...
    if failed_entities:
        output_message += "\n\nAction wasn't able to stop the process with {} {} from the following entities: {}".format(
            pid_type,
            pid_value,
    ...
    if failed_entities_with_reason:
        for item in failed_entities_with_reason:
            output_message += f"\n\nAction wasn't able to stop the process with {pid_type} {pid_value} from {item.get('entity')}."
            if item.get("reason"):
...
def main(is_first_run):
    ...
    pid_type = extract_action_param(
        siemplify,
        param_name="Process Identifier Type",
        is_mandatory=True,
        input_type=str,
    )
    pid_value = extract_action_param(
        siemplify, param_name="Process Identifier", is_mandatory=True, input_type=str
    )
    ...
    try:
        ...
        if is_first_run:
            output_message, result_value, status = start_operation(
                siemplify, mvision_edr_manager, pid_type, pid_value
            )
        else:
            (
                processed_entities,
