...
from ..core.constants import INTEGRATION_NAME, PREVENT_FILE_SCRIPT_NAME, MD5
...
def main():
    ...
    try:
            ...
            try:
                ...
                if hash_type != MD5:
                    ...
                    siemplify.LOGGER.info(
                        f"Hash {entity_identifier} is not supported. Supported type is MD5"
                    )
                    continue
        ...
        if successful_entities:
            output_message = (
                f"Successfully added the following hashes to the blacklist in {INTEGRATION_NAME}: "
                f'{", ".join(successful_entities)}\n'
            )
            if failed_entities:
                output_message += (
                    f"Action wasn't able to add the following hashes to the blacklist in "
                    f'{INTEGRATION_NAME}: {", ".join(failed_entities)}\n'
                )
        else:
            result_value = False
            output_message = (
                f"No hashes were added to the blacklist in {INTEGRATION_NAME}."
            )

    except Exception as e:
        output_message = (
            f"Error executing action {PREVENT_FILE_SCRIPT_NAME}. Reason: {e}"
        )
