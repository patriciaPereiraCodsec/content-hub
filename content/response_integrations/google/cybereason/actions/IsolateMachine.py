            ...
            if machine is not None:
                if machine.isolated:
                    siemplify.LOGGER.info(
                        f"The following machine already isolated: {entity_identifier}"
                    )
...
def query_operation_status(
    ...
    for entity_identifier, machine_pylum_id in result_data["for_isolate"].items():
        ...
        if is_approaching_timeout(action_start_time, DEFAULT_TIMEOUT):
            siemplify.LOGGER.info(
                f"Action processed {len(completed_machines.keys())} out of {items_to_isolate_count}"
            )
            break
        ...
        try:
            ...
            siemplify.LOGGER.info(
                f"Machine with {machine_pylum_id} pylum id isolated: {entity_identifier}"
            )
    ...
    if result_data["for_isolate"] and not timeout_approaching:
        ...
        output_message = (
            f"Waiting for isolation to finish on the following entities: "
...
def finish_operation(suitable_entities, result_data, timeout_approaching):
    ...
    if successful_entities:
        output_message += (
            f"Successfully isolated the following machines in {INTEGRATION_NAME}: "
            f"{', '.join(successful_entities)}. \n"
        )
    if failed_entities:
        output_message += (
            f"Action wasn't able to isolate the following machines in {INTEGRATION_NAME}: "
    ...
    if timeout_approaching and not_finished:
        output_message += (
            f"Isolation was initiated on the following entities, but wasn't finished: "
...
def main(is_first_run):
    ...
    try:
        ...
        output_message = (
            f"Error executing action {ISOLATE_MACHINE_SCRIPT_NAME}. Reason: {err}"
        )
        result_value = False
        status = EXECUTION_STATE_FAILED
