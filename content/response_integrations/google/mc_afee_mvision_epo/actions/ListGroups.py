...
from ..core.constants import LIST_GROUPS_SCRIPT_NAME, INTEGRATION_NAME, DEFAULT_LIMIT_GROUPS
...
def main():
    ...
    max_groups_to_return = extract_action_param(
        ...
        is_mandatory=False,
        input_type=int,
        default_value=DEFAULT_LIMIT_GROUPS,
    )
    ...
    try:
        ...
        if groups:
            ...
            siemplify.result.add_data_table(
                title="Available Groups", data_table=construct_csv(csv_table)
            )
            output_message += (
                "Successfully listed available groups in McAfee Mvision ePO."
            )

        else:
            result_value = False
            output_message += (
                "Action wasn’t able to list groups available in McAfee Mvision ePO."
            )
        ...
        output_message += f"Error executing action List Groups. Reason: {e}"
        result_value = False
