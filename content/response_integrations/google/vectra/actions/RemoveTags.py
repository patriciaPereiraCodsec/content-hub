...
from ..core.constants import INTEGRATION_NAME, REMOVE_TAGS_SCRIPT_NAME
    ...
    try:
        ...
        if not successfully_removed_tags:
            raise TagsUpdateFailException(
                f"Tags {tags} don't exist in {item_type} with ID {item_id}"
            )

        vectra_manager.update_tags(item_type, item_id, existing_tags)
        output_message = (
            f"Successfully removed tags {','.join(successfully_removed_tags)} from"
            f" {item_type} with ID {item_id}"
        )
        ...
        output_message = (
            f"Action wasn't able to remove tags from {item_type} with ID {item_id}"
        )
        result_value = "false"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(output_message)
    except Exception as e:
        output_message = f'Error executing action "Remove Tags". Reason: {e}'
        result_value = "false"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
