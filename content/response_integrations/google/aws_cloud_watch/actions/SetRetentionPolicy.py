...
from ..core.consts import INTEGRATION_DISPLAY_NAME, SET_RETENTION_POLICY
...
def main():
    ...
    try:
        retention_days = extract_action_param(
            siemplify,
        ...
        siemplify.LOGGER.info(
            f"Setting new retention policy for log group {log_group_name}.."
        )
        manager.set_retention_policy(
            log_group_name=log_group_name, retention_in_days=retention_days
        )

        output_message = (
            f"Successfully set the retention policy for log group {log_group_name} in "
