...
def main():
    ...
    try:
        ...
        if mitigation_details:
            output_message = (
                "Successfully fetched details of incident mitigations for the "
                f"{time_period}"
            )
            result_value = True
            siemplify.result.add_result_json(
                [detail.to_json() for detail in mitigation_details]
            )
        else:
            output_message = (
                f"No details of incident mitigations were found for the {time_period}"
            )

    except Exception as e:
        output_message = (
            f'Error executing action "Get Incident Mitigation Details". Reason: {e}'
        )
