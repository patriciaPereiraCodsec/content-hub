    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                siemplify.LOGGER.info(
                    f"Successfully blocked the following IP {entity.identifier}"
                )
        ...
        if failed_entities:
            output_message += (
                "Action was not able to block the following IPs in the iBoss category "
                "with ID {}: \n{}\n"
            ).format(
        ...
        if enriched_entities:
            output_message += (
                "Successfully blocked the following IPs in the iBoss category with ID "
                "{} \n{}\n"
            ).format(
                category_id,
            ...
            output_message = (
                f"No IPs were blocked in the iBoss category with ID {category_id}."
            )
        ...
        output_message = (
            f"Error executing action '{ADD_IP_TO_POLICY_BLOCK_LIST}'. Reason: {e}"
        )
