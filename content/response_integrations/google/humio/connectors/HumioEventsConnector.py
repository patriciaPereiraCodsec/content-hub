...
def main(is_test_run):
    ...
    api_token = extract_connector_param(
        siemplify, param_name="API Token", is_mandatory=True
    )
    ...
    repository_name = extract_connector_param(
    ...
    alert_field_name = extract_connector_param(
        siemplify, param_name="Alert Field Name", print_value=True
    )
    severity_field_name_string = extract_connector_param(
        siemplify, param_name="Severity Field Name", is_mandatory=True, print_value=True
    )
    severity_mapping_json_string = extract_connector_param(
        siemplify,
        param_name="Severity Mapping JSON",
        is_mandatory=True,
        print_value=True,
    )
    ...
    try:
        ...
        try:
            ...
            raise Exception(
                'Invalid JSON object is provided in parameter "Severity Mapping JSON". Make sure that the'
        ...
        if not is_test_run:
            ...
            save_timestamp(
                siemplify=siemplify, alerts=fetched_alerts, timestamp_key="timestamp"
            )
