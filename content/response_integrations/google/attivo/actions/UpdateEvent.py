...
def main():
    ...
    try:
        ...
        if event_status == SELECT_ONE and not comment:
            raise Exception(
                'at least one of the parameters "Status" or "Comment" should have a value.'
            )
        ...
        output_message += (
            f"Error executing action {UPDATE_EVENT_SCRIPT_NAME}. Reason: {e}"
        )
