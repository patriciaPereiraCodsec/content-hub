...
from ..core.constants import EXECUTE_FILE_SCRIPT_NAME, INTEGRATION_NAME
from ..core.Factory import ManagerFactory
from ..core.AsyncActionStepHandler import (
    ActionStep,
    FAILED_ENTITY_STATE,
    COMPLETED_ENTITY_STATE,
...
ACTION_COMPLETE_OUTPUT_MESSAGES = {ALL_FAILED_MSG: "No files were executed."}

FAILED_OUTPUT_MESSAGE = (
    "Action failed to execute a file {filename} for the following entities: {entities}"
)
SUCCESSFUL_OUTPUT_MESSAGE = (
    "Successfully executed  file {filename} for the following entities: {entities}"
)
...
def main():
    ...
    output_log_file = extract_action_param(
        siemplify, param_name="Output Log File on Remote Host", print_value=True
    )
    command_arguments = extract_action_param(
        siemplify, param_name="Command Arguments to Pass to File", print_value=True
    )
    wait_for_result = extract_action_param(
        siemplify, param_name="Wait for the Result", print_value=True, input_type=bool
    )
    ...
    try:
        ...
        output_message = (
            f"Failed to execute {EXECUTE_FILE_SCRIPT_NAME} action! Error is {e}"
        )
            ...
            if command.is_completed or command.is_failed:
                ...
                if command.is_completed:
                    ...
                    json_result = collect_json_result(
                        step_handler, item, command, filename, json_result
                    )
                else:
...
def get_output_message(step_handler):
    ACTION_COMPLETE_OUTPUT_MESSAGES.update(
        {
            SOME_COMPLETED_MSG: format_output_message(
                step_handler, SUCCESSFUL_OUTPUT_MESSAGE, COMPLETED_ENTITY_STATE
            ),
