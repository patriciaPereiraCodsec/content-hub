...
from ..core.constants import (
    LIST_PROCESSES_SCRIPT_NAME,
    INTEGRATION_NAME,
    DEFAULT_RESULTS_LIMIT,
    PROCESSES_CASE_WALL,
    VENDOR_NAME,
)
from ..core.utils import get_entity_original_identifier, get_validated_limit
...
def main():
    ...
    max_results = extract_action_param(
        siemplify,
        param_name="How Many Records To Return",
        print_value=True,
        input_type=int,
    ...
    try:
        ...
        output_message = (
            f"Failed to execute {LIST_PROCESSES_SCRIPT_NAME} action! Error is {e}"
        )
...
def add_custom_output_messages(step_handler):
    ...
    if step_handler.get_custom_variable(FAILED_SESSION_INIT):
        message += (
            f"Failed to initiate Live Response session for the following entities: "
            f"{', '.join(step_handler.get_custom_variable(FAILED_SESSION_INIT))}\n"
        )

    if step_handler.get_custom_variable(FAILED_COMMAND_INIT):
        message += (
            f"Failed to run command via Live Response session for the following entities: "
