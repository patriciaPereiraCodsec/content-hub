...
def main():
    ...
    for entity in target_entities:
        try:
            ...
            error_message = f'Error isolating "{entity.identifier}", Error: {str(err)}'
            siemplify.LOGGER.error(error_message)
            siemplify.LOGGER.exception(err)
            errors.append(error_message)

    if command_ids_csv:
        output_message = f"{','.join([entity.identifier for entity in isolated_entities])} isolated successfully."
