"""MarkCaseInProgress action for Abnormal Security Google SecOps SOAR Integration.
...
from ..core.constants import INTEGRATION_NAME, MARK_CASE_IN_PROGRESS_SCRIPT_NAME


@output_handler
def main() -> None:
    """Main execution logic for the MarkCaseInProgress action."""
    siemplify = SiemplifyAction()
    siemplify.script_name = MARK_CASE_IN_PROGRESS_SCRIPT_NAME
    siemplify.LOGGER.info(f"Action: {MARK_CASE_IN_PROGRESS_SCRIPT_NAME} started")
    ...
    try:
        ...
        for cid in case_ids:
            try:
                response = manager.post_case_action(case_id=cid, action="acknowledge_in_progress")
                successes.append(cid)
                aggregated[cid] = response
            except Exception as inner:
                failures.append((cid, str(inner)))
                siemplify.LOGGER.error(f"Case {cid} acknowledge_in_progress failed: {inner}")

        siemplify.result.add_result_json(aggregated)
        if successes and not failures:
            output_message = (
                f"Successfully marked cases as 'in progress' on the following entities using Abnormal Security: "
                f"{', '.join(successes)}"
            )
            result_value = True
            ...
            output_message = (
                f"Successfully marked cases as 'in progress' on the following entities using Abnormal Security: "
                f"{', '.join(successes)}. "
                f"Action wasn't able to mark cases as 'in progress' on the following entities using "
        ...
        output_message = f'Error executing action "{MARK_CASE_IN_PROGRESS_SCRIPT_NAME}". Reason: {e}'
