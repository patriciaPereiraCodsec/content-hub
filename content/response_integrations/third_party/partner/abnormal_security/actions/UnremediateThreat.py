"""UnremediateThreat action for Abnormal Security Google SecOps SOAR Integration.
...
from ..core.constants import INTEGRATION_NAME, UNREMEDIATE_THREAT_SCRIPT_NAME


@output_handler
def main() -> None:
    """Main execution logic for the UnremediateThreat action."""
    siemplify = SiemplifyAction()
    siemplify.script_name = UNREMEDIATE_THREAT_SCRIPT_NAME
    siemplify.LOGGER.info(f"Action: {UNREMEDIATE_THREAT_SCRIPT_NAME} started")
    ...
    try:
        ...
        for tid in threat_ids:
            try:
                response = manager.post_threat_action(threat_id=tid, action="unremediate", message_ids=message_ids)
                successes.append(tid)
                aggregated[tid] = response
            except Exception as inner:
                failures.append((tid, str(inner)))
                siemplify.LOGGER.error(f"Threat {tid} unremediate failed: {inner}")

        siemplify.result.add_result_json(aggregated)
        if successes and not failures:
            output_message = (
                f"Successfully unremediated threats on the following entities using Abnormal Security: "
                f"{', '.join(successes)}"
            )
            result_value = True
            ...
            output_message = (
                f"Successfully unremediated threats on the following entities using Abnormal Security: "
                f"{', '.join(successes)}. "
                f"Action wasn't able to unremediate threats on the following entities using Abnormal Security: "
                f"{failed_ids}"
            )
            result_value = True
            status = EXECUTION_STATE_COMPLETED
        ...
        output_message = f'Error executing action "{UNREMEDIATE_THREAT_SCRIPT_NAME}". Reason: {e}'
