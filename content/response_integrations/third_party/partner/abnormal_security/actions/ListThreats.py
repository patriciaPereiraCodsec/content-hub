"""List Threats action for Abnormal Security Google SecOps SOAR Integration."""
...
from ..core.constants import INTEGRATION_NAME, LIST_THREATS_SCRIPT_NAME


@output_handler
def main() -> None:
    """Main execution logic for the List Threats action."""
    siemplify = SiemplifyAction()
    siemplify.script_name = LIST_THREATS_SCRIPT_NAME
    siemplify.LOGGER.info(f"Action: {LIST_THREATS_SCRIPT_NAME} started")
    ...
    try:
        ...
        output_message = f"Retrieved {len(threats)} threat(s)."
        result_value = True
        ...
        output_message = f'Error executing action "{LIST_THREATS_SCRIPT_NAME}". Reason: {e}'
