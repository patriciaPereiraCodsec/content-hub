...
class RRSSession(MockSession[MockRequest, MockResponse, RansomwareResilience]):
    ...
    @router.post(r".*/storage/take-volume-online")
    def volume_online_endpoint(self, request: MockRequest) -> MockResponse:
        """Handle volume online requests."""
        status_code = self._product.volume_online_status_code
        if status_code and status_code >= 400:
            body = self._product.volume_online_response or {"error": "request_failed"}
            return MockResponse(content=body, status_code=status_code)
        return MockResponse(content=self._product.get_volume_online(), status_code=200)
